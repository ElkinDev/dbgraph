...
### Requirement: Honest minimal dependency hints for views and triggers

The SQLite adapter SHALL derive dependency hints for views and triggers only where they are trivially
and honestly obtainable from `sqlite_master.sql`. Where a body cannot be reliably parsed, the object
MUST be marked `has_dynamic_sql: true` (US-007) rather than guessing references. Full SQL-body
dependency parsing is explicitly DEFERRED beyond Phase 2 (first delivered by the SQL Server adapter,
US-027, Phase 3).

#### Scenario: Unparseable view/trigger body is flagged not guessed

- GIVEN a view or trigger whose body cannot be reliably parsed into references
- WHEN the adapter extracts it
- THEN the object is marked `has_dynamic_sql: true`
- AND no speculative `reads_from`/`writes_to`/`depends_on` edge is fabricated

#### Scenario: Full body parsing is deferred past Phase 2

- GIVEN the Phase 2 SQLite adapter
- WHEN dependency hints are produced
- THEN only catalog-trivial hints are required of it
