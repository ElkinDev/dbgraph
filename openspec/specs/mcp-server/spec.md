...
manual configuration snippet and exit successfully rather than fail. Path resolution MUST be
host-independent: `pathWin32.join` for the Windows branch, `pathPosix.join` for the Linux/macOS branch,
so the function returns identical output regardless of where Node.js runs.
