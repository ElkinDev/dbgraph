...
|9|`dbgraph install`|`resolveConfigPath(platform, env)`: win `%APPDATA%\Claude\claude_desktop_config.json`, linux `~/.config/Claude/…`; idempotent `mergeMcpConfig` adds `mcpServers.dbgraph-mcp`; `--remove` deletes only that key; print manual snippet when path absent. fs/path injected as a seam|spawn an installer|pure-ish core unit-testable; US-024 minimum viable (Claude only)|
...
- [x] Exact pinned `@modelcontextprotocol/sdk` version + its `StdioServerTransport`/`InMemoryTransport`/`CallToolResult` shape — RESOLVED in Batch B (task 2.5): `1.29.0`; low-level `Server` chosen over `McpServer`/Zod; documented in `docs/learnings.md`.
- [x] Final token budgets — RESOLVED in Batch E (task 5.4): empirically measured, all TBD replaced in `docs/format-spec.md`.
