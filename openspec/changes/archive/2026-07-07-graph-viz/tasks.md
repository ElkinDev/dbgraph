...
- [x] `dbgraph viz` writes ONE self-contained HTML (all JS/CSS inline, ZERO network at view time) that opens over `file://` with force layout, pan/zoom and dark theme. — Batch 3 (3.2, 3.6), Batch 4 (4.2 manual)
- [x] Nodes are colored by COMMUNITY (fallback `kind`); the sidebar legend lists community name + count with per-community visibility toggles. — Batch 1 (1.2), Batch 4 (4.2 manual)
- [x] Search + kind/degree/schema filters + column-collapse keep the ~18.7k graph usable; the full graph is explicit opt-in (`--full`), honestly documented. — Batch 1 (1.1), Batch 3 (3.1), Batch 4 (4.1, 4.2)
- [x] Clicking a node opens a detail panel whose content MATCHES `dbgraph object <qname>` (same `formatObject` source, no second renderer). — Batch 1 (1.5), Batch 4 (4.2 manual)
- [x] Community ASSIGNMENT + the embedded data block + `--mermaid` output are byte-deterministic and golden-pinned; the live animation is NOT goldened (ADR-008 split). — Batch 1 (1.2, 1.4, 1.6), Batch 3 (3.6)
- [x] `dbgraph viz --mermaid` emits a valid deterministic ER diagram (tables + FKs). — Batch 1 (1.6), Batch 3 (3.3)
- [x] The generated file embeds NO connection strings/secrets/sampled values; docs state it is as sensitive as the graph DB. — Batch 3 (3.6), Batch 4 (4.1)
- [x] The `GraphStore` bulk read is strictly read-only (ADR-004 boundary green), bounded (≤2 whole-graph, ≤3 total store calls), driver-agnostic; `viz` is NOT registered as an MCP tool. — Batch 2 (2.3, 2.4), Batch 3 (3.7)
- [x] Vendored `d3-force`/`d3-quadtree` are inlined assets (ADR-010, PROVENANCE version+sha256) with ZERO new npm runtime dependency, shipped inside the SEA blob for offline standalone binaries. — Batch 3 (3.5, 3.8)
- [x] `npx tsc --noEmit` strict clean (NO `any`); `npm run lint` 0/0; `npm test` green with count strictly ≥ the recorded baseline; all viz + storage goldens byte-identical (ADR-008). — Batch 4 (4.3)
