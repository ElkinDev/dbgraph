# Delta for Benchmark — v2 Task-Planning Decision-Quality Measurement

Delta spec for change `benchmark-v2-task-planning`. All six requirements below are
**MODIFIED** against the canonical `openspec/specs/benchmark/spec.md` (re-anchored to
current text). Each MODIFIED block is the FULL requirement — the archive step REPLACES
the canonical requirement with the block below, so unchanged scenarios are copied intact.

| # | Amends canonical requirement | Anchor | Proposal delta |
|---|------------------------------|--------|----------------|
| 1 | Question set is mechanically derived and carries machine-checkable ground truth | spec.md:47 (rule (c) at :53) | Req-2 hand-planted carve-out |
| 2 | Scoring is deterministic, blind, unit-tested, and separates closed-form from rubric | spec.md:130 | Third scoring rule (topo-order) |
| 3 | Reproducible-first, dual substrate with the fixture load-bearing | spec.md:26 | Reproducible-with-Docker tier |
| 4 | Multiple runs are reported as code-version-labeled tables on the frozen protocol | spec.md:230 | Own-labeled v2 run / frozen set |
| 5 | WITHOUT-dump coverage is machine-asserted at build time | spec.md:293 (table :328) | Coverage table + plan families kind-agnostic |
| 6 | The report carries all limitations alongside results and forbids extrapolation | spec.md:164 | Limitations additions |
...
(Previously: all families' ground truth was mechanically derived — rule (c) took rename-impact keys
straight from `affected`; there was no exception and no hand-planted key path.)
...
(Previously: only two pinned rules existed — exact-match and unordered set-match; there was no ordering
rule and no topological-order comparator.)
...
(Previously: only two tiers existed — the reproducible SQLite primary and the non-reproducible private
mssql secondary; there was no reproducible-with-Docker middle tier.)
...
(Previously: every additional table was treated as a RE-RUN of the single frozen SQLite set and forbidden
from adding any command, question, or scoring rule; there was no concept of a NEW substrate-labeled set
carrying its own pre-registered families and additively-extended scorer.)
...
match. A schema object name is unique within its schema, so a name-only match cannot yield a FALSE pass on
the correct substrate; a name genuinely ABSENT from a wrong-DB dump still MISSES and aborts. The failure
output MUST name the missing OBJECT and the qid, and MUST NOT contain any ground-truth key VALUE — a bare
schema OBJECT identifier is safe (it already appears un-redacted in the dump), whereas a COMPOSED answer
value (e.g. a full FK path) is NOT.
(Previously: only IMPACT-derived targets were matched KIND-AGNOSTICALLY; the plan-* planning families did
not exist in the target-derivation table.)
...
(Previously: the Limitations enumeration did not include the five v2 task-planning limitations
— plan-quality-unscored, format-prompting bias, hand-planted-key judgment risk, statically-decidable
radius, and Docker-tier reproducibility.)
