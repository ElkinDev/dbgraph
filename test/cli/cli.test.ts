...
describe('cli — --version / -v (task 1.1, design D6)', () => {
  ...
  it('DBGRAPH_VERSION placeholder is still exactly "1.1.1" (fallback anchor for the smoke)', () => {
    expect(DBGRAPH_VERSION).toBe('1.1.1');
  });

  it('runCli(["--version"]) with DBGRAPH_BUILD_VERSION unset prints EXACTLY "1.1.1\\n" and returns 0', async () => {
    const code = await runCli(['--version']);
    expect(code).toBe(0);
    expect(stdout.join('')).toBe('1.1.1\n');
  });

  it('runCli(["-v"]) with DBGRAPH_BUILD_VERSION unset prints EXACTLY "1.1.1\\n" and returns 0', async () => {
