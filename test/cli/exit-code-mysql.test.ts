 ...
 * (listing 'mysql') did NOT perturb the mapping, WITHOUT editing exit-code.ts.
 *
 * This test proves:
 *   1. UnsupportedDialectError('mysql') maps to 4 (new message, mysql listed)
 ...
 *   (verified unchanged)". Design §8: "add explicit instanceof code-4 regression-guard
...
describe('exitCodeFor — UnsupportedDialectError → 4 (instanceof guard, task 5.4)', () => {
  ...
  it('UnsupportedDialectError("mysql") maps to exit code 4 (mysql message does not perturb mapping)', () => {
    // After task 5.3 the message now lists "sqlite, mssql, pg, mysql".
  ...
  it('UnsupportedDialectError message now contains "sqlite, mssql, pg, mysql"', () => {
  ...
  it('exit code 4 is stable regardless of the unsupported dialect name', () => {
    ...
    const dialects = ['mysql', 'redis', 'mongodb', 'oracle'];
