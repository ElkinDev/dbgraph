...
**Phase:** 8 · **Depends on:** US-027 (patterns established) · **Status:** ☐ pending

**Acceptance criteria:**
- Extracts, beyond the common set: materialized views, custom types/enums, sequences, RLS policies (as `metadata`), comments (`obj_description`).
- Dependencies via `pg_depend`/`pg_rewrite`; functions and triggers (incl. `CREATE TRIGGER ... EXECUTE FUNCTION`).
- `docs/permissions/postgres.md` with the minimal read-only role.
