...
## 2026-06-18 — FOR JSON renders sql_variant as native JSON types; coerce, do not requireString (F-9)
- **Context:** Phase-6 `sync` via the sqlcmd/manual-dump FOR JSON path. `coerceSequenceRow` used `requireString` for `start_value`, `increment`, `minimum_value`, `maximum_value` (all `sys.sequences` columns typed `sql_variant`). `coerceExtendedPropRow` used `requireString` for `description` (`sys.extended_properties.value` is also `sql_variant`).
- **What failed:** FOR JSON serialises `sql_variant` columns as their NATIVE JSON type — numeric sequences produce JSON numbers (e.g. `1000620314`), not quoted strings. `requireString` throws `"required string field ... is missing or not a string: got 1000620314"` and blocks sync entirely. The tedious path is unaffected because tedious deserialises sql_variant to strings; only the FOR JSON path is affected.
- **Root cause:** `sql_variant` is a polymorphic SQL Server type. The canonical string representation (e.g. `"1"`, `"9223372036854775807"`) is what the type-cast version of the query produces (CAST to nvarchar). When the query does NOT include an explicit CAST, FOR JSON emits the underlying primitive type verbatim.
- **Fix:** Added `coerceStringy(value, fieldName): string` helper: passes strings through, coerces finite `number` via `String()`, coerces `bigint` via `.toString()`, coerces `boolean` via `String()`, throws on null/undefined/object. Applied `coerceStringy` to the four sequence fields and `description`.
